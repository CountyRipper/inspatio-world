...
# Experimental V3.2: yaw-only 0 -> 45 -> 0 -> 45. The first exact +45
# frame is the sole DA3 keyframe/point-cloud write; the final +45 is read-only.
# For 247 and 300 frames, those first peaks are RGB indices 82 and 100.
...
GPU="${GPU:-2}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/data4/daixiangting/inspatio_v3_2_results}"
...
run_case() {
    ...
    /usr/bin/time -v -o "${output_dir}/pipeline_resource_time.txt" \
        ...
        --memory_single_keyframe_index "$keyframe_index" \
    ...
    python scripts/summarize_overlap_voxel_v3.py \
        "${result_dir}/0-memory_timing_rank0.json" \
        --csv "${OUTPUT_ROOT}/${case_name}_v3_2_block_timings.csv" \
        --summary-json "${OUTPUT_ROOT}/${case_name}_v3_2_summary.json"
}
