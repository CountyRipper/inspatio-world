...
mkdir -p "${RESULT_ROOT}"
trap 'code=$?; if [[ $code -eq 0 ]]; then printf "complete\n" > "${STATUS_FILE}"; else printf "failed exit_code=%s\n" "$code" > "${STATUS_FILE}"; fi' EXIT
cd "${PROJECT_ROOT}"

printf 'running example0\n' > "${STATUS_FILE}"
GPU="${GPU}" MASTER_PORT=29562 \
...
./run_scripts/run_slam3r_offline_v6_2_end2end.sh

printf 'running example1\n' > "${STATUS_FILE}"
GPU="${GPU}" MASTER_PORT=29563 \
